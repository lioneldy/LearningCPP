...
----
- 简介
- 马尔科夫过程及隐式马尔科夫过程的介绍，引出隐式马尔科夫三个基本问题
- 如何解决隐式马尔科夫模型三个基本问题
- 隐式马尔科夫模型的几种类型
- 隐式马尔科夫模型的具体实现问题
- 利用隐式马尔科夫模型实现语音识别
...
#### 问题3的解法
----
为了描述迭代更新参数模型的过程，我们引入变量$\xi_t(i,j)$表示在已给参数变量$\lambda$和观测序列$O$的情况下，$t$时刻状态为$S_i$且$t+1$时刻状态为$S_j$的概率，即$$\xi_t(i,j)=P(q_t=S_i,q_{t+1}=S_j\mid O,\lambda)$$在之前我们已经给过$\gamma_t(i)$的定义，由定义我们可知二者有以下关系$$\gamma_t(i)=\sum_{j=1}^N\xi_t(i,j)$$利用这两个变量，我们可以描述下列期望$$\sum_{t=1}^{T-1}\gamma_t(i)=从状态S_i转移到其它状态的期望次数$$$$\sum_{t=1}^{T-1}\xi_t(i,j)=从状态S_i转移到状态S_j的期望次数$$
...
在本节中，我们处理几个实际的实现问题，包括缩放，多个观察序列，初始参数估计，缺少的数据，以及模型大小和类型的选择
